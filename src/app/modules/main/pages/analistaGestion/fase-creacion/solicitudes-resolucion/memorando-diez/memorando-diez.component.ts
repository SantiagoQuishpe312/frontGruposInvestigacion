...
@Component({
  selector: 'vex-memorando-diez',
  templateUrl: './memorando-diez.component.html',
  styleUrls: ['./memorando-diez.component.scss']
})
export class MemorandoDiezComponent implements OnInit {
  ...
  setFileName() {
    ...
    const name = `memo_recomendacion_consejo_acad_${this.groupId}_${this.currentDate.getFullYear()}-${this.currentDate.getMonth() + 1}-${this.currentDate.getDate()}.pdf`;
      ...
      this.invGroupService.update(this.groupId, invGroup).subscribe(
        (response) => {
          localStorage.removeItem('GI');
            this.router.navigate(["main/solicitudes-aprobadas"]);
        });
    })
  }
