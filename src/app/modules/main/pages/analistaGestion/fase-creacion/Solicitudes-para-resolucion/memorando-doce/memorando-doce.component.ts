...
@Component({
  selector: 'vex-memorando-doce',
  templateUrl: './memorando-doce.component.html',
  styleUrls: ['./memorando-doce.component.scss']
})
export class MemorandoDoceComponent implements OnInit {
  ...
  setFileName() {
    ...
    const name = `memo_registro_GI_${this.groupId}_${this.currentDate.getFullYear()}-${this.currentDate.getMonth() + 1}-${this.currentDate.getDate()}.pdf`;
      ...
      this.invGroupService.update(this.groupId, invGroup).subscribe(
        (response) => {
          localStorage.removeItem('GI');
            this.router.navigate(["main/grupos-resoluciones"]);
        });
    })
    this.creationReqService.getByGroup(this.groupId).subscribe((data) => {
      const creationReqForm: CreationReqForm = {
