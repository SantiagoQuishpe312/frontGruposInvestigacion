...
@Component({
  selector: 'vex-memorando36',
  templateUrl: './memorando36.component.html',
  styleUrls: ['./memorando36.component.scss']
})
export class Memorando36Component implements OnInit {
selectedFile: File | undefined;
  ...
  setFileName() {
    ...
    const name = `actas_${this.groupId}_${this.currentDate.getFullYear()}-${this.currentDate.getMonth() + 1}-${this.currentDate.getDate()}.pdf`;
      ...
      this.invGroupService.update(this.groupId, invGroup).subscribe(
        (response) => {
          localStorage.removeItem('GI');
            this.router.navigate(["main/solicitudesA"]);
        });
    })
  }
