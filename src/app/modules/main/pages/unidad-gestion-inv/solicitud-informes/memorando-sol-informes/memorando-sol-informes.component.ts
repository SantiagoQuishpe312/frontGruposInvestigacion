...
@Component({
  selector: 'vex-memorando-sol-informes',
  templateUrl: './memorando-sol-informes.component.html',
  styleUrls: ['./memorando-sol-informes.component.scss']
})
export class MemorandoSolInformesComponent implements OnInit {
  ...
  selectedFile: File | undefined;
    ...
    setFileName() {
      ...
      const name = `memo_delega_revision_informe_unidad_resp_GI_${this.groupId}_${this.currentDate.getFullYear()}-${this.currentDate.getMonth() + 1}-${this.currentDate.getDate()}.pdf`;
      ...
      this.invGroupService.getById(this.groupId).subscribe(data => {
        ...
        this.invGroupService.update(this.groupId, invGroup).subscribe(
          (response) => {
            ...
            this.router.navigate(["main/solicitudes-informes"]);
          });
      })
    }
