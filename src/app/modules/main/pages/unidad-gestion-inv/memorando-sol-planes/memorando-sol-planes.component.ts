...
@Component({
  selector: 'vex-memorando-sol-planes',
  templateUrl: './memorando-sol-planes.component.html',
  styleUrls: ['./memorando-sol-planes.component.scss']
})
export class MemorandoSolPlanesComponent implements OnInit {
  ...
  setFileName() {
    ...
    const name = `memo_delega_revision_planes_unidad_resp_GI_${this.groupId}_${this.currentDate.getFullYear()}-${this.currentDate.getMonth() + 1}-${this.currentDate.getDate()}.pdf`;
    ...
    this.invGroupService.getById(this.groupId).subscribe(data => {
      ...
      this.invGroupService.update(this.groupId, invGroup).subscribe(
        (response) => {
          ...
          this.router.navigate(["main/solicitudes-planes"]);
        });
    })
  }
