...
@Component({
  ...
  templateUrl: './resolucionConsejo.component.html',
  styleUrls: ['./resolucionConsejo.component.scss']
})
export class ResolucionConsejoComponent implements OnInit {
  ...
  setFileName() {
    ...
    const name = `resolucion_consejo_carrera_GI_${this.groupId}_${this.currentDate.getFullYear()}-${this.currentDate.getMonth() + 1}-${this.currentDate.getDate()}.pdf`;
   ...
   if(resolucion==='favorable'){
    this.invGroupService.getById(this.groupId).subscribe(data=>{
      ...
      this.invGroupService.update(this.groupId,invGroup).subscribe(
        (response)=>{
          this.router.navigate(["main/memorando-vitt"]);        });
    })
   }
