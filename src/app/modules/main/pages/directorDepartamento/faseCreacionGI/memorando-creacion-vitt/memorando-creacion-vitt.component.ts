...
@Component({
  selector: 'vex-memorando-creacion-vitt',
  templateUrl: './memorando-creacion-vitt.component.html',
  styleUrls: ['./memorando-creacion-vitt.component.scss']
})
export class MemorandoCreacionVITTComponent implements OnInit {
  ...
  setFileName() {
    ...
    const name = `memorando_solicitud_vitt_GI_${this.groupId}_${this.currentDate.getFullYear()}-${this.currentDate.getMonth() + 1}-${this.currentDate.getDate()}.pdf`;
    ...
    this.invGroupService.getById(this.groupId).subscribe(data=>{
      ...
      this.invGroupService.update(this.groupId,invGroup).subscribe(
        (response)=>{
          ...
          this.router.navigate(["main/coord2"]);        });
    })
